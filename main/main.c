...
// ==================== PLATFORM-SPECIFIC CONFIGURATION ====================
    ...
    #define TXD_PIN_PC GPIO_NUM_18
    #define RXD_PIN_PC GPIO_NUM_19
    #define VOLTAGE_MULTIPLIER 2.0
#endif
...
#define SEND_INTERVAL_MS 500           // Send EVERY 500ms (continuous logs)
...
void uart_init(void) {
    ...
    // CRITICAL: Test UART by sending initial message
    const char* test_msg = "UART2 Communication Started\n";
    uart_write_bytes(UART_PC_NUM, test_msg, strlen(test_msg));
#endif
}
...
// NEW: Send to BOTH monitor and PC
void sendLog(const char *data) {
    // Send to ESP32 monitor (USB)
    ESP_LOGI(TAG, "%s", data);
...
// ==================== BELT MONITORING TASK (CONTINUOUS LOGGING) ====================
// Only send structured data to PC, logs stay on monitor
void belt_monitor_task(void *pvParameters) {
    ...
    ESP_LOGI(TAG, "Baseline: %lu mV", (unsigned long)voltage);
    ...
    while (1) {
        ...
        if (!first_drop_detected && voltage_diff > VOLTAGE_THRESHOLD) {
            ...
            ESP_LOGI(TAG, "FIRST LOOP: %lu mV (drop=%d)", (unsigned long)voltage, voltage_diff);
            ...
            if (interval > CHECK_INTERVAL_MS / portTICK_PERIOD_MS) {
                ...
                sendDatatoPC(uart_buf);
                ESP_LOGI(TAG, "LOOP OK: I=%lu ms", (unsigned long)interval_ms);
